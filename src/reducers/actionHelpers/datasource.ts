...
import { DataSource } from "@/records/index";
...
export const setPartialTreeData = (
    state,
    ...
    const pathToNode = [-1, ...getTreePathFromId(flat, parentId)];
    const updatedTree = setTreeValue(tree, pathToNode, { children: data });
...
export const removeRow = (state, { stateKey, rowIndex }) => {
    ...
    const updatedTotal =
        existingState && currentTotal && currentTotal > 0
            ? currentTotal - 1
            : 0;
...
export const updateRow = (state, { rowIndex, stateKey, values }) => {
    ...
    const row = data ? data.get(rowIndex) : null;
...
export const addNewRow = (state, { rowId, stateKey, rowIndex }) => {
    ...
    let newRow =
        data && data.size > 0 && data.get(0)
            ? data.get(0).reduce((p, i, c) => {
                  return p.set(c, "");
              }, fromJS({}))
            : fromJS({});
    ...
    const updatedTotal =
        existingState && existingState.get("total")
...
export const setTreeNodeVisibility = (
    ...
    const currentVisibility = !!flat
        .find((node) => node.get("_id") === id)
        .get("_hideChildren");

    const path = [-1, ...getTreePathFromId(flat, id)];

    const updatedTree = setTreeValue(tree, path, {
        _hideChildren: !currentVisibility,
    });
...
export const saveRow = (state, { rowIndex, stateKey, values }) => {
    const data = state.getIn([stateKey, "data"]).set(rowIndex, fromJS(values));

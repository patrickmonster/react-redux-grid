...
export class ActionColumn extends Component {
    render() {
        ...
        const { maxHeight, menuPosition } = this.state;
        ...
        const containerProps = {
            className: prefix(
                CLASS_NAMES.GRID_ACTIONS.CONTAINER,
...
export const enableActions = (menuShown, actions, columns, rowData) => {
    if (menuShown && actions && typeof actions.onMenuShow === "function") {
        const val = rowData && rowData.toJS ? rowData.toJS() : rowData;
...
export const getHeader = (
    ...
    if (!headerActionItemBuilder) {
        colActions = columns.map((col) => {
            const isChecked = col.hidden !== undefined ? !col.hidden : true;
        ...
        colActions = columns.map(
            headerActionItemBuilder.bind(null, {
                store,
                columns,
            })
        );
...
export const handleHideMenu = (
    ...
    const isRowAction =
        elementContains(e.target, prefix(CLASS_NAMES.ROW)) &&
        e.target.classList.contains(prefix(CLASS_NAMES.GRID_ACTIONS.ICON));
    ...
    const actionDisabled =
        e.target &&
        e.target.classList.contains(prefix(CLASS_NAMES.GRID_ACTIONS.DISABLED));

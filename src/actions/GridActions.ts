...
import { setLoaderState } from "./plugins/loader/LoaderActions";

import { dismissEditor } from "./plugins/editor/EditorActions";
...
export const getAsyncData = ({
    ...
    return (dispatch) => {
        ...
        dispatch(setLoaderState({ state: true, stateKey }));
        ...
        if (typeof dataSource === "function") {
            ...
            dataSource(extraParams).then((response) => {
                if (response && response.data) {
                    dispatch(setLoaderState({ state: false, stateKey }));
...
export const setSortDirection = ({ columns, id, sortDirection, stateKey }) => {
    let cols = columns;
...
export const insertRow = ({ stateKey, data, index = 0 }) => ({
    type: INSERT_ROW,
    stateKey,
    data,
    index,
...
export const moveNode = ({ stateKey, current, next, showTreeRootNode }) => ({
    type: MOVE_NODE,
    stateKey,
    current,
    next,

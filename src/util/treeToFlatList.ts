...
export const treeToFlatList = (
    ...
    if (data.get(rootIdentifier)) {
        ...
        stack = stack.push(toItem(List(), childIdentifier, cfg)(data));
    } else {
        stack = data
            .get(childIdentifier)
            .map(toItem(List([-1]), List([0]), childIdentifier));
    }
    ...
    while (stack.count()) {
        ...
        result.push(
            item.delete(childIdentifier).delete("parentId").delete("id")
        );
    }
...
const toItem =
    ...
    (node, index = 0) => {
        const previousSibling =
            index - 1 > -1 ? siblings.get(index - 1) : undefined;

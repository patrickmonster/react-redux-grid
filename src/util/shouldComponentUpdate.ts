import { getLastUpdate } from "@/util/lastUpdate";
...
export function shouldPagerUpdate(nextProps, nextState) {
    ...
    result = !deepEqual(limitedNextProps, limitedProps);
...
export const isEdited = (editorState, key) =>
    Boolean(editorState && editorState.get(key) && editorState.get(key).values);
